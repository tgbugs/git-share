# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: 1ada98c5818a0b6c88201b2c3406482c1f56fd9d620ec808dfa1c47f38f18a97; -*-
...
#+name: parse-args-asd
#+header: :eval never
#+begin_src lisp :noweb yes :tangle (when (boundp '*-gsell-*) "./parse-args.asd")
(in-package :cl-user)
(defpackage :parse-args-asd (:use #:cl #:asdf))
(in-package :parse-args-asd)
(defsystem parse-args
  :version "0.0.1"
  :author "Tom Gillespie <tgbugs@gmail.com>"
  :license "GPL2"  ; match the git license need to check compat with other things here
  :description "stupidly simple command line argument parsing"
...
(defun process-bind-keyword (bind-keyword)
  (let* ((kw-or-element? (car bind-keyword))
         ...
         (kw (car element))
         (sl (and (keywordp kw) (string-downcase (symbol-name kw))))
         (subcmd? (not (keywordp kw)))
         (assign? (and (not subcmd?) (cdr element)))
         (real-assign (if bind? (car bind?) (if subcmd? kw (intern sl 'parse-args))))
    ...
    (list `(,real-assign ,default)  ; default
          `(,(if subcmd? kw (intern (format nil "--~a" sl) 'parse-args)) ,p)  ; case
          `(cons ',real-assign ,real-assign))))

(defmacro parse-args (&body keywords)
  `(ow-cli-gen ,keywords parsed))

(defmacro cli-gen (bind-keywords &rest body) ; (ref:cli-gen)
...
((:option default) option-internal) -> --option value -> (let ((option-internal \"value\")) )"
  ...
  (destructuring-bind (defaults cases returns)
      (apply #'mapcar #'list
             (mapcar #'process-bind-keyword bind-keywords))
    `(let ((args (cdr (uiop:raw-command-line-arguments)))
           ,@defaults)
       (do ()
           ((null args) nil)
         (case (intern (car args) 'parse-args)
           ,@cases
...
(defmacro parse-args-old (&body keywords)
...
*** deployment
**** ccl
Can't use =:native= on linux, only on darwin.

#+name: git-share-ccl
#+begin_src sh :shebang "#!/usr/bin/env sh" :tangle ./bin/git-share-ccl
ccl -I "${0}.image"
#+end_src

#+name: &build-image-ccl
#+begin_src lisp :noweb yes
<<quicklisp-setup>>
...
(save-application "bin/git-share-ccl.image" :toplevel-function #'main :native nil)
#+end_src

#+name: build-image-ccl
#+header: :shebang "#!/usr/bin/env sh"
#+begin_src sh :noweb yes :tangle ./bin/build-image-ccl :mkdirp yes
read -r -d '' CODE <<'EOF'
<<&build-image-ccl>>
EOF
CODE_PATH=$(mktemp)
echo "${CODE}" > "${CODE_PATH}"
/usr/bin/env -S ccl --no-init --load "${CODE_PATH}"
rm "${CODE_PATH}"
#+end_src
**** SLaD
...
# NOTE dexador requires libss libcrypto and libz
# so on gentoo if you have portage then you have what you need
# in all likelihood explicitly emerge -1q openssl sys-libs/zlib
...
git-share-first-commit () {
    FCS=$(git rev-list --max-parents=0 HEAD)
...
(defvar ow-do-tangle nil "Override this for emacs --batch")

(defvar ow-do-build nil "Override this for emacs --batch")

(defvar ow-do-packages-system nil
  "Install system packages.")

(defvar ow-do-packages-emacs nil
...
(unless (featurep 'ow-min)
  (reval 'sha256 '3282d9328e05a12595ffc13e5afbeb3b3a76f1e3c45d50a53df39c2afd6ca341
         "https://raw.githubusercontent.com/tgbugs/orgstrap/1e1fa08f9c1c059b393f737fcf19b55082bf660/ow-min.el"))
...
(defun orgstrap---packages ()
  "nothing doing")

;; system packages go first in case elisp packages depend on them
(when ow-do-packages-system
  (orgstrap---system))

(when (or ow-do-packages-system ow-do-packages-emacs)
  (orgstrap---packages))

(when (or ow-do-tangle ow-do-build)
  (org-babel-tangle)
  (when ow-do-build
    (orgstrap---make)))

;; TODO org-babel-do-load-languages
(unless (alist-get 'lisp org-babel-load-languages)
...
(condition-case nil
    (unless (cl-some #'featurep '(slime sly))
      (require 'slime))
  (error (package-install 'slime)))

(when noninteractive
  ;;(load "~/git/NOFORK/org-mode/lisp/ob-core.el")
  (require 'ob-core)
  (let ((org-confirm-babel-evaluate-cell (lambda (cell) (message "%s" cell) nil)) ; SIGH
